...
class MotorDockSE3Model(nn.Module):
    def __init__(self, protein_feat_dim: int, ligand_feat_dim: int, pair_feat_dim: int, hidden_dim: int = 256, num_layers: int = 4, dropout: float = 0.1, use_pair_attention: bool = True, use_motor_auxiliary: bool = True, disable_pair_context: bool = False, freeze_baseline_encoder: bool = False, max_pair_rotation_scale: float = 0.5, max_pair_translation_scale: float = 5.0):
        super().__init__()
        ...
        self.motor = MotorAdapterSE3(hidden_dim, hidden_dim, max_pair_rotation_scale, max_pair_translation_scale)
        ...
        if freeze_baseline_encoder:
            for m in [self.protein_enc, self.ligand_enc]:
    ...
    def forward(self, batch: dict) -> dict:
        ...
        motor_out = self.motor(pair_h, l_ctx, p_ctx, batch["pair_T_input"], batch["pair_mask"])

        if self.use_pair_attention:
            pair_ctx, pair_attn = self.pair_pool(pair_h, l_ctx + p_ctx, batch["pair_mask"])
        else:
            denom = batch["pair_mask"].float().sum(dim=1, keepdim=True).clamp_min(1.0)
            pair_attn = batch["pair_mask"].float() / denom
